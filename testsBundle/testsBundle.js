...
${this.logColors.Reset}`
  ...
  function tokenize({ source, current, start, line, tokens }) {
    ...
    let lineCursor = line;
    const createToken_ = (tokenType, cursorShift2 = 0) => createToken({
      tokenType,
      source,
      lexemeStart,
      cursor: cursor + cursorShift2,
      line: lineCursor
    });
    const cursorShift = 1;
    const createMultiCharToken = (tokenType) => createToken_(tokenType, cursorShift);
    const addToken_ = (tokenType, cursorShift2 = 0) => newTokens.push(createToken_(tokenType, cursorShift2));
    while (cursor < source.length) {
      ...
      if (currentCharacter === "\n") {
        ...
        if (match(source, cursor, "=")) {
          ...
          newTokens.push(createToken_(TOKENS.TOK_NOT));
      }
      lexemeStart = cursor;
    }
    return {
